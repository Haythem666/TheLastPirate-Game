...
#func _process(delta):
	...
	#
	## Vérifie tous les corps dans l'Area2D
	#for body in $Area2D.get_overlapping_bodies():
		#if body is RigidBody2D or body.name == "PlayerTest":
			#
			#button_pressed = true
			#break
#
	## Si l'état change, toggle le bouton
	#if button_pressed and state == "0":
		#_toggle_button()
	#elif not button_pressed and state == "1":
		#_toggle_button()
#
#func _toggle_button():
	## Changer l'état
...
#
	...
	#_update_button_appearance()
#
	## Jouer animation de la plateforme
	#_play_platform_anim()
#
#func _update_button_appearance():
	#if state == "0":
		#$NotPressedSprite.visible = true
		#$PressedSprite.visible = false
	#else:
		#$NotPressedSprite.visible = false
...
#func _play_platform_anim():
		...
		#if state == "1":
			#anim_player.play("downPlateform")
		#else:
			#anim_player.play("upPlateform")
